...
with TransactionTestingCommons {
  ...
  def cleanTransactionPool(): Unit = untilTimeout(1.second) {
    LagonakiUnconfirmedTransactionsDatabase.all().foreach(tx => LagonakiUnconfirmedTransactionsDatabase.remove(tx))
    LagonakiUnconfirmedTransactionsDatabase.all().size shouldBe 0
  }
  ...
  test("Generate block with plenty of transactions") {
    val block = untilTimeout(1.minute) {
      stopGeneration()
      val toGen = LagonakiUnconfirmedTransactionsDatabase.SizeLimit - transactionModule.packUnconfirmed().size
  ...
  test("Don't include same transactions twice") {
    ...
    incl.foreach(tx => LagonakiUnconfirmedTransactionsDatabase.putIfNew(tx.asInstanceOf[LagonakiTransaction]))
    LagonakiUnconfirmedTransactionsDatabase.all().size shouldBe incl.size
    val tx = genValidTransaction(randomAmnt = false)
    LagonakiUnconfirmedTransactionsDatabase.all().size shouldBe incl.size + 1
